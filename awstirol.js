...
const markeroptions = {title: "Automatische Wetterstationen von Tirol",     //http://leafletjs.com/reference-1.3.0.html#marker-title
    draggable : false,                   //http://leafletjs.com/reference-1.3.0.html#marker-draggable
    opacity : 0.9,                       //http://leafletjs.com/reference-1.3.0.html#marker-opacity
  }

//fuer jede Station, erstelle einen Marker mit jeweils Name, Temperatur, Datum, Link
let i;
for (i in awsdata) {
  // test for loop --> console.log(awsdata[i]["name"]);
  let awsdatastring =awsdata[i]["link"];
  awsdatastring = awsdata[i]["name"] + "</p>"+
  "<p>Temperatur [°C]: "+awsdata[i]["temperatur"]+"</p>"+
  "<p>Datum: "+awsdata[i]["datum"]+"</p>"+
  "<p>Link: "+awsdatastring.link(awsdata[i]["link"]);

  L.marker([awsdata[i]["lat"],awsdata[i]["lng"]], markeroptions).addTo(markerGroup).bindPopup(awsdatastring);
}
